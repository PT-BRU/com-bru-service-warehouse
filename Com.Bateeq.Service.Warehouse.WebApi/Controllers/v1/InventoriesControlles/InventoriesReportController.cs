...
namespace Com.MM.Service.Warehouse.WebApi.Controllers.v1.InventoryControllers
{
    ...
    public class InventoryReportController : Controller
    {
        ...
        public IActionResult GetStockByPeriod(string storage, DateTime dateFrom, DateTime dateTo, string group, string category, string style, string collection, string season, string color, string sizes, string zero, string info, int page = 1, int size = 100, string Order = "{}")
        {
            int offset = Convert.ToInt32(Request.Headers["x-timezone-offset"]);
            identityService.Username = User.Claims.Single(p => p.Type.Equals("username")).Value;
            string accept = Request.Headers["Accept"];
            ...
            try
            {
                var data = facade.GetStockByPeriod(storage, dateFrom, dateTo, group, category, style, collection, season, color, sizes,zero, page, size);
        ...
        public IActionResult GetStockByPeriodXls(string storage, DateTime dateFrom, DateTime dateTo, string group, string category, string style, string collection, string season, string color, string sizes, string zero)
        {
            try
            {
                byte[] xlsInBytes;
                ...
                var xls = facade.GetXLSStockByPeriod(storage, dateFrom, dateTo, group, category, style, collection, season, color, sizes,zero);
