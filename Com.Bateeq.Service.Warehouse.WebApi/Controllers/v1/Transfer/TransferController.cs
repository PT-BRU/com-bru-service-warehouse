...
namespace Com.Bateeq.Service.Warehouse.WebApi.Controllers.v1.Transfer
{
    ...
    public class TransferController : Controller
    {
        ...
        [HttpGet("received")]
        public IActionResult GetReceived(int page = 1, int size = 25, string order = "{}", string keyword = null, string filter = "{}")
        {
            try
            {
                identityService.Username = User.Claims.Single(p => p.Type.Equals("username")).Value;

                var Data = facade.ReadWarehouseReceived(page, size, order, keyword, filter);
